    ...
    <body>
        ...
        <script>
            ...
            function getStatusText(status) {{
                const statusMap = {{
                    'pending': '{t.get('status_pending', 'Pending')}',
                    'validating': '{t.get('status_validating', 'Validating')}',
                    'processing': '{t.get('status_processing', 'Processing')}',
                    'completed': '{t.get('status_completed', 'Completed')}',
                    'failed': '{t.get('status_failed', 'Failed')}'
                }};
            ...
            function updateResultDisplay(job, selectedModel) {{
                ...
                if (job.status === 'completed') {{
                    html += `
                        ...
                        <p style="color: #4CAF50;">✓ {t.get('processing_complete', 'Processing complete! Results ready for download')}</p>
                        <button class="download-btn" onclick="downloadResult('${{job.id}}')">📥 {t.get('download_results', 'Download Results')}</button>
                    `;
                }} else if (job.status === 'failed') {{
                    html += `
                        <p style="color: #f44336;">✗ {t.get('processing_failed', 'Processing failed')}</p>
                    ...
                    html += `
                        ...
                        <p style="color: #666;">{t.get('processing_wait', 'Processing... Please wait')}</p>
                    `;
                }} else {{
                    html += `
                        <div class="progress-bar"><div class="progress-fill" style="width: 20%"></div></div>
                        <p style="color: #666;">{t.get('waiting_in_queue', 'Waiting in queue...')}</p>
                    `;
                }}
            ...
            async function downloadResult(jobId) {{
                try {{
                    const response = await fetch(`{get_url('/api/v1/jobs/')}${{jobId}}/download`);
                    ...
                    alert('{t.get('download_failed', 'Download failed')}: ' + error.message);
                }}
            }}
            ...
            document.getElementById('uploadForm').onsubmit = async function(e) {{
                ...
                resultDiv.className = 'info';
                resultDiv.innerHTML = '<p>{t.get('uploading_file', 'Uploading file')}...</p>';
                resultDiv.style.display = 'block';
